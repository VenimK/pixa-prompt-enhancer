...
Extracted from app/gemini.py and refactored to implement the ModelProvider interface.
"""
...
class GeminiProvider(ModelProvider):
    ...
    def generate_text(
        ...
        return self._call_gemini(prompt, image_path=None, image_paths=None, model_override=model_override)
    ...
    def generate_with_image(
        ...
        return self._call_gemini(prompt, image_path=image_path, image_paths=None, model_override=model_override)
    ...
    def generate_with_images(
        ...
        return self._call_gemini(prompt, image_path=None, image_paths=image_paths, model_override=model_override)
    ...
    def evaluate_quality(
        ...
        return _evaluate_enhanced_prompt_quality(
            enhanced_prompt=enhanced_prompt,
            prompt_type=prompt_type,
            model=model,
            gemini_provider=self,
        )
...
async def run_gemini_async(
    ...
    """Async wrapper for legacy code compatibility."""
    provider = GeminiProvider()
    return await asyncio.to_thread(provider._call_gemini, prompt, image_path, image_paths, model_override)


# ---------------------------------------------------------------------------
# Prompt quality scoring (Gemini-specific)
# ---------------------------------------------------------------------------
...
def _evaluate_enhanced_prompt_quality(
    enhanced_prompt: str,
    prompt_type: str | None = None,
    model: str | None = None,
    gemini_provider: GeminiProvider | None = None,
) -> dict | None:
    """Evaluate prompt quality using Gemini."""
    ...
    try:
        provider = gemini_provider or GeminiProvider()
        raw_result = provider.generate_text(scoring_prompt)
